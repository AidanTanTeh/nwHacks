...
import { Play, Pause, Square, MapPin, Zap, Clock, X } from 'lucide-react';
...
const WorkoutTracker: React.FC<WorkoutTrackerProps> = ({ workoutType, onFinish, onCancel }) => {
  ...
  const needsGPS = workoutType === WorkoutType.RUN || workoutType === WorkoutType.WALK || workoutType === WorkoutType.CYCLING;
  ...
  const startWorkout = useCallback(() => {
    ...
    if (needsGPS && 'geolocation' in navigator) {
  ...
  const stopWorkout = () => {
    ...
    let stats: { distance?: number; duration: number; pace?: string } = {
      duration
    };

    if (needsGPS && distance > 0) {
      const pace = (duration / 60) / distance;
      const paceMin = Math.floor(pace);
      const paceSec = Math.round((pace - paceMin) * 60);
      stats.distance = distance;
      stats.pace = `${paceMin}:${paceSec.toString().padStart(2, '0')}`;
    }
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-white relative overflow-hidden transition-colors duration-300">
      ...
      {/* Header */}
      <div className="z-10 pt-8 px-6 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <span className="text-4xl">{getWorkoutIcon()}</span>
          <div>
            <h1 className="text-xl font-black tracking-tight">{workoutType}</h1>
            {needsGPS && (
              <div className={`flex items-center gap-2 mt-1 text-xs font-bold tracking-wider uppercase ${errorMsg ? 'text-red-600 dark:text-red-400' : 'text-zinc-500 dark:text-zinc-400'}`}>
                <MapPin size={12} />
      ...
      <div className="flex-1 z-10 flex flex-col items-center justify-center p-6 space-y-10">
        ...
        {needsGPS ? (
          <div className="text-center">
            <div className="text-[8rem] leading-[0.85] font-black tracking-tighter tabular-nums text-zinc-900 dark:text-white">
              {distance.toFixed(2)}
            </div>
          ...
          <div className="text-center">
            ...
            <div className="text-zinc-500 font-bold tracking-[0.2em] uppercase text-sm mt-2">Duration</div>
          </div>
        )}

        {/* Secondary Stats */}
        <div className={`grid ${needsGPS ? 'grid-cols-2' : 'grid-cols-1'} gap-12 w-full max-w-xs`}>
          {needsGPS && (
            <>
      ...
      <div className="z-10 pb-36 px-8 w-full flex justify-center items-center">
        {trackingState === TrackingState.IDLE && (
          <button 
            ...
            className="w-24 h-24 bg-orange-500 rounded-full flex items-center justify-center shadow-[0_4px_30px_-5px_rgba(249,115,22,0.6)] active:scale-90 transition-transform"
          >
        ...
        {trackingState === TrackingState.RUNNING && (
          <button 
            ...
            className="w-24 h-24 bg-white dark:bg-zinc-800 border-2 border-zinc-200 dark:border-zinc-700 rounded-full flex items-center justify-center active:scale-90 transition-transform shadow-lg"
          >
        ...
        {trackingState === TrackingState.PAUSED && (
          <div className="flex items-center gap-8 animate-in fade-in slide-in-from-bottom-4">
            <button 
              onClick={startWorkout}
              className="w-20 h-20 bg-white dark:bg-zinc-800 rounded-full flex items-center justify-center border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700 active:scale-90 transition-transform shadow-md"
            >
            ...
            <button 
              onClick={stopWorkout}
              className="w-24 h-24 bg-orange-500 rounded-full flex items-center justify-center shadow-[0_4px_40px_-5px_rgba(249,115,22,0.5)] active:scale-95 transition-transform"
            >
