...
import { Home, Play, BarChart2, User as UserIcon, Users } from 'lucide-react';
...
const Navigation: React.FC<NavigationProps> = ({ currentView, onChangeView }) => {
  const navItems = [
    { view: AppView.FEED, icon: Home, label: 'Home' },
    { view: AppView.LEADERBOARD, icon: BarChart2, label: 'Rank' },
    { view: AppView.RUN, icon: Play, label: 'Run', highlight: true },
    { view: AppView.FRIENDS, icon: Users, label: 'Friends' },
            ...
            <div className="flex justify-around items-end h-20 px-2 pb-2">
                ...
                if (item.highlight) {
                    return (
                        <div key={item.view} className="relative -top-6">
                            ...
                            >
                                <Icon size={28} fill="currentColor" className="ml-1" />
                            </button>
                        </div>
                    );
                }
                ...
                return (
                    ...
                    className={`flex flex-col items-center justify-center gap-1 w-14 h-12 mb-2 transition-colors duration-200 ${
                        isActive ? 'text-orange-500 dark:text-white' : 'text-zinc-400 dark:text-zinc-600 hover:text-zinc-600 dark:hover:text-zinc-400'
                    }`}
                    >
